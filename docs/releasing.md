...
- **Excluded from the package:** everything listed in `.distignore` (the
  ...
  `.wordpress-org/` (`.org` page assets, deployed separately to SVN `assets/`),
...
into a `divine-apparitions-uploads-proxy/` folder (the `.org` slug) and running
Plugin Check, e.g. `wp plugin check divine-apparitions-uploads-proxy`. The text
domain matches the slug, so there should be **no** `WordPress.WP.I18n.TextDomainMismatch`
and **no** readme or plugin-header "cannot find" errors (see ADR-0003).
...
5. Tag the release and publish it on GitHub: `git tag vX.Y.Z`, push the tag, then
   create a GitHub Release for it. Publishing the Release triggers `deploy.yml`,
   which pushes `trunk` + `tags/X.Y.Z` to WordPress.org SVN automatically.
...
> repository name (`wp-uploads-proxy`); `deploy.yml` already sets
> `slug: divine-apparitions-uploads-proxy` explicitly so it targets the right repo.
> When validating locally with Plugin Check, extract into a folder named
> `divine-apparitions-uploads-proxy/` too, so the checker derives the matching text
> domain.
...
- `.github/workflows/deploy.yml`: when a **GitHub Release is published**,
  `10up/action-wordpress-plugin-deploy` (pinned to a commit SHA) builds the package
  per `.distignore` and pushes it to SVN `trunk` and `tags/X.Y.Z`. The `.org` slug
  is set to `divine-apparitions-uploads-proxy` (the action otherwise defaults to the
  GitHub repo name), and a leading `v` on the tag is stripped so the SVN tag matches
  `readme.txt` `Stable tag`.
- `.github/workflows/assets.yml`: on push to `master` touching `.wordpress-org/`
  (or via `workflow_dispatch`), `10up/action-wordpress-plugin-asset-update` (pinned
  to a commit SHA) pushes the `.org` page assets (banner, icon, screenshots) to the
  SVN `assets/` directory (outside `trunk`). Put the images in `.wordpress-org/` —
  see its `README.md` for names and sizes. `.wordpress-org/` is excluded from
  `.distignore` and `.gitattributes`, so it is never copied into `trunk` or the
  shipped package.
...
> Status: **0.11.1 is live** at `wordpress.org/plugins/divine-apparitions-uploads-proxy`
> (first release uploaded via SVN manually). The asset pipeline is validated — a
> `workflow_dispatch` run of `assets.yml` deployed to SVN `assets/` successfully.
> Future releases publish through `deploy.yml` on a GitHub Release.
