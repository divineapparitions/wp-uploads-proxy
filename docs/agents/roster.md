...
after seeding** — trim entries that don't apply to this repo (this is a
WordPress plugin, so the Drupal / devenv / native-app agents are safe to drop).
Re-running `triage --setup` re-seeds the file from the current set of installed agents.
